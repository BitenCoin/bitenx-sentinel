...
def test_valid_bitenx_address():
    from bitenxlib import is_valid_bitenx_address

    main = valid_bitenx_address()
    test = valid_bitenx_address('testnet')

    assert is_valid_bitenx_address(main) is True
    assert is_valid_bitenx_address(main, 'mainnet') is True
    assert is_valid_bitenx_address(main, 'testnet') is False

    assert is_valid_bitenx_address(test) is False
    assert is_valid_bitenx_address(test, 'mainnet') is False
    assert is_valid_bitenx_address(test, 'testnet') is True


def test_invalid_bitenx_address():
    from bitenxlib import is_valid_bitenx_address

    main = invalid_bitenx_address()
    test = invalid_bitenx_address('testnet')

    assert is_valid_bitenx_address(main) is False
    assert is_valid_bitenx_address(main, 'mainnet') is False
    assert is_valid_bitenx_address(main, 'testnet') is False

    assert is_valid_bitenx_address(test) is False
    assert is_valid_bitenx_address(test, 'mainnet') is False
    assert is_valid_bitenx_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from bitenxlib import elect_mn
...
def test_parse_masternode_status_vin():
    from bitenxlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % bitenxlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(bitenxlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(bitenxlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(bitenxlib.blocks_to_seconds(16616)) == 2612035

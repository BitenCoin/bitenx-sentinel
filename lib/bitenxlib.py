...
def is_valid_bitenx_address(address, network='mainnet'):
    ...
    bitenx_version = 140 if network == 'testnet' else 76
    ...
    if (address_version != bitenx_version):
        return False
...
# shims 'til we can fix the bitenxd side
def SHIM_serialise_for_bitenxd(sentinel_hex):
    ...
    # superblock => "trigger" in bitenxd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # bitenxd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    bitenxd_hex = serialise(obj)
    return bitenxd_hex


# shims 'til we can fix the bitenxd side
def SHIM_deserialise_from_bitenxd(bitenxd_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(bitenxd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('bitenx.conf')

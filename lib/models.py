...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, bitenxd):
        golist = bitenxd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_bitenxd(bitenxd, item)
    ...
    @classmethod
    def import_gobject_from_bitenxd(self, bitenxd, rec):
        import decimal
        import bitenxlib
        import inflection
        ...
        # shim/bitenxd conversion
        object_hex = bitenxlib.SHIM_deserialise_from_bitenxd(object_hex)
        objects = bitenxlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from bitenxd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from bitenxd, with every run
        ...
        # get/create, then sync payment amounts, etc. from bitenxd - Dashd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(bitenxd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from bitenxd! %s" % e)
            govobj.vote_delete(bitenxd)
            return (govobj, None)
    ...
    def vote_delete(self, bitenxd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(bitenxd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, bitenxd, signal, outcome):
        import bitenxlib
        ...
        output = bitenxd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = bitenxlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(bitenxd, signal)

    def sync_network_vote(self, bitenxd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = bitenxd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 bitenx addr, non-multisig
            if not bitenxlib.is_valid_bitenx_address(self.payment_address, config.network):
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (bitenxlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def get_prepare_command(self):
        import bitenxlib
        obj_data = bitenxlib.SHIM_serialise_for_bitenxd(self.serialise())
    ...
    def prepare(self, bitenxd):
        try:
            object_hash = bitenxd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not bitenxlib.is_valid_bitenx_address(addr, config.network):
    ...
    def hash(self):
        import bitenxlib
        return bitenxlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, bitenxd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - bitenxd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, bitenxd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - bitenxd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, bitenxd):
        now = int(time.time())
        return (self.created_at < (now - bitenxd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, bitenxd):
        if self.is_expired(bitenxd):
            return False

        return True

    def is_deletable(self, bitenxd):
        if self.is_expired(bitenxd):
            return True

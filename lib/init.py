...
def has_bitenx_conf():
    ...
    valid_bitenx_conf = False

    # ensure bitenx_conf exists & readable
    #
    # if not, print a message stating that Dash Core must be installed and
    # configured, including JSONRPC access in bitenx.conf
    try:
        f = io.open(config.bitenx_conf)
        valid_bitenx_conf = True
    except IOError as e:
        print(e)

    return valid_bitenx_conf
...
def main():
    ...
    if not has_bitenx_conf():
        print("DashCore must be installed and configured, including JSONRPC access in bitenx.conf")
        sys.exit(1)
